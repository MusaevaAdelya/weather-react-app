...
function SearchInput({ city, setCity, handleSearch, setIsError }) {
  return (
    <form className="search" onSubmit={(e) => handleSearch(e)}>
      ...
      />
      <button type="submit" className="bi bi-search search__icon"></button>
    </form>
  );
}
  ...
  return (
    <div>
      <div className="current">
        ...
        <p className="current__atmosphere">{data.weather[0].description}</p>
        <p className="current__info">Feels like {data.main.feels_like}°C</p>
        <p className="current__info">
          <i className="bi bi-wind color-accent"></i> {data.wind.speed} m/s{" "}
          <i
      ...
      <div className="date">
        ...
        <p className="date__day">{getCurrentDateForTimezone(data.timezone)}</p>
      </div>
    </div>
  );
}
